...
--  v12 Optimized - single shared loop, event-driven chams
...
local HP_BAR_OFF    = Vector2.new(5, 0)
local HP_TXT_OFF    = Vector2.new(3, 0)
local HP_BAR_OL_OFF = Vector2.new(0, 1)
local NAME_OFF      = Vector2.new(0, 2)
local DIST_OFF      = Vector2.new(0, 2)
...
local function getBoundingBox(parts)
    ...
    return CFrame.new(center, Vector3.new(center.X, center.Y, mx.Z)), mx - mn
end

local function worldToScreen(world)
    local s, inBounds = wtvp(camera, world)
    return Vector2.new(s.X, s.Y), inBounds, s.Z
end
...
local _frame     = 0
local UPDATE_SKIP  = 3  -- Update() runs every 3rd frame   20 Hz
local CHAMS_SKIP   = 6  -- SyncHighlight() runs every 6th frame  10 Hz
...
function EspObject:Render()
    ...
    if vis.hpBar.Visible then
        ...
        local pct   = math.max(0, mMin(1, self.health / self.maxHealth))
        vis.hpBar.To    = bTo
    ...
    if vis.tracer.Visible then
        ...
        tr.From = opts.tracerOrigin=="Top"    and vp*Vector2.new(0.5,0)
               or opts.tracerOrigin=="Middle" and vp*0.5
               or vp*Vector2.new(0.5,1)
...
local EspInterface = {
    ...
    teamSettings = {
        enemy = {
            ...
            headDot              = false,
            headDotColor         = {Color3.new(1,1,1), 0},
        },
        friendly = {
            enabled              = false,
            box                  = false,
...
--  3. MATH & FAST LOCALS
-- ==============================================================================
local mFloor, mClamp, mMax, mMin, mSqrt = mFloor, mClamp, mMax, mMin, math.sqrt
local mRand = mRand
...
local HP_PAL = {Color3.fromRGB(255,50,50), Color3.fromRGB(255,130,0), Color3.fromRGB(255,200,0), Color3.fromRGB(100,255,50), Color3.fromRGB(0,255,100)}
local function HPC(p)
    if p <= 0 then return HP_PAL[1] end; if p >= 1 then return HP_PAL[5] end
    local s = p*4+1; local i = math.floor(s); local f = s-i
...
local _limbPool = {}
local _aimOff   = Vector3.new(0,0,0)
...
local loadSound = iNew("Sound"); loadSound.SoundId = "rbxassetid://4590657391"; loadSound.Volume=2
...
local function BuildCC(pl, char)
    ...
    task.spawn(function()
        ...
        if hrp and head and hum then
            ...
            c._sp = Vector3.new(); c._onSc=false; c._depth=0; c._distSq=0; c._wsValid=false
            CC[pl] = c
            UpdateTBRayFilter()
        end
    end)
...
local function GetAimPart(cd, mode, camP, fovP, cam)
    ...
    elseif mode == "Nearest Part" then
        ...
        for i=1, #SMART_P do
            ...
            if p then
                ...
                if on then
                    local d = (Vector2.new(sp.X,sp.Y)-fovP).Magnitude
    ...
    elseif mode=="Limbs" then
        ...
        part = #_limbPool>0 and _limbPool[math.random(1,math.max(1,#_limbPool))] or cd.HRP
...
local function TickFOV(ctr, cam, dt)
    ...
    local fov = cam.FieldOfView; if fov <= 0 then fov = 70 end
    local scale = S.FOV.ZoomScale and (70/fov) or 1
    local targetR = math.max(1, S.FOV.Radius * scale)
    
    if _currFovR == 0 then _currFovR = targetR end
    _currFovR = _currFovR + (targetR - _currFovR) * math.clamp(dt * 15, 0, 1)
            ...
            if cd.Hum.Health<=0 or not tar.Parent or not cd.Char or not cd.Char.Parent or not cd._wsValid then ClearTarget()
                ...
                if part then
                    ...
                    if S.Aim.HitChance >= 100 or math.random(1,100)<=S.Aim.HitChance then
                        if S.Aim.On and S.Aim.IsAiming then
                            if S.Aim.Smooth and not snap then
                                local sf = math.clamp((S.Aim.SmoothSpd^1.5)*(dt*60), 0.01, 1)
                    ...
                    if S.Aim.LockTracer and cd._onSc then
                        setL(LTracer, Vector2.new(sw*0.5,sh*0.5), Vector2.new(cd._sp.X,cd._sp.Y), lockVis and WHITE or RED, 1.5, 1)
                    else if LTracer.Visible then LTracer.Visible=false end end


                    local maxHP  = math.max(1, cd._maxHP or 100)
                    local hp     = math.clamp(cd.Hum.Health, 0, maxHP)
                    local pct    = hp / maxHP
                    local hpFlr  = math.floor(hp)
                    local dstFlr = math.floor(distToTarget)
                    ...
                    if hpFlr ~= _thudLastHP or dstFlr ~= _thudLastDist or tar.DisplayName ~= _thudLastName then
                        ...
                        THUD.Txt.Text = ("Target: %s  |  HP: %d/%d  |  %dm"):format(tar.DisplayName, hpFlr, math.floor(maxHP), dstFlr)
                    end
                    local w = math.max(260, THUD.Txt.TextBounds.X+30); local hh=26; local x=(sw-w)*0.5; local y=sh-100
                    setRect(THUD.BG,    Vector2.new(w,hh),            Vector2.new(x,y),    Color3.fromRGB(20,20,25), 10, 0, 0.85, true)
                    THUD.Txt.Position = Vector2.new(sw*0.5, y+5); THUD.Txt.Visible=true
                    setRect(THUD.BarBG, Vector2.new(w,4),             Vector2.new(x,y+hh), Color3.fromRGB(40,40,45), 11, 0, 1, true)
                    setRect(THUD.Bar,   Vector2.new(math.max(1,w*pct),4), Vector2.new(x,y+hh), HPC(pct),     12, 0, 1, true)
                end
            end
        end
    else
...
local function TickHB(dt)
    do -- World/FOV overrides (guarded)
        ...
        if S.World.FullB then
            if Lit.Ambient ~= Color3.new(1,1,1) then Lit.Ambient = Color3.new(1,1,1) end
            if Lit.OutdoorAmbient ~= Color3.new(1,1,1) then Lit.OutdoorAmbient = Color3.new(1,1,1) end
            if Lit.ColorShift_Bottom ~= Color3.new(1,1,1) then Lit.ColorShift_Bottom = Color3.new(1,1,1) end
            if Lit.ColorShift_Top ~= Color3.new(1,1,1) then Lit.ColorShift_Top = Color3.new(1,1,1) end
        end
    end
    local mc = CC[LP]
    ...
    if S.HB.On then
        ...
        if _hbTimer >= 0.1 then
            _hbTimer = 0
            local newSz=Vector3.new(S.HB.Size,S.HB.Size,S.HB.Size); local newTr=S.HB.Trans
            local _plCount = #PList
...
local function TickRender(dt)
    ...
    local fovP = TickFOV(Vector2.new(_cX, _cY), cam, dt)
...
local Window = Library:CreateWindow({
    Title = "KAIM v12",
    Footer = "v12 | github.com/sswird/kaim",
    Icon = 95816097006870,
    NotifySide = "Right",
...
local KaimWatermark = Library:AddDraggableLabel("KAIM v12 | FPS: 0 | Ping: 0ms")
...
ThemeManager:SetFolder("KAIM_v12")
SaveManager:SetFolder("KAIM_v12/configs")
...
Conns[#Conns+1] = RS.Heartbeat:Connect(function()
    ...
    if now - lastUIRefresh >= 0.5 then
        ...
        KaimWatermark:SetText("KAIM v12 | FPS: " .. tostring(fps) .. " | Ping: " .. math.floor(ping) .. "ms")
        lastUIRefresh = now
    end
end)
...
Library:Notify({Title="KAIM Obsidian v12", Description="Loaded - Obsidian Edition", Time=4})
...
if hookmetamethod then
    ...
    if not checkcaller() and S.Silent.On and S.Silent.Target then
        if method == "Raycast" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist" or method == "FindPartOnRay" then
            ...
            if tc and tc.Head then
                local hitPart = tc.Head
                if S.Silent.HeadshotChance < 100 and math.random(1, 100) > S.Silent.HeadshotChance then
                    hitPart = tc.HRP or tc.Head
                end
                
                if math.random(1, 100) <= S.Silent.HitChance then
