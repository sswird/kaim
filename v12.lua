    ...
    THUD.Container.Size = UDim2.new(0, 200, 0, 52)
    THUD.Container.Position = UDim2.new(0, -9999, 0, -9999) -- Hidden off-screen initially
    ...
    bgStroke.Color = Color3.fromRGB(45, 45, 45)
    ...
    avCorner.CornerRadius = UDim.new(0, 2) -- Sharp tactical square
    avCorner.Parent = THUD.Avatar

    local avStroke = Instance.new("UIStroke")
    avStroke.Color = Color3.fromRGB(50, 50, 50)
    avStroke.Transparency = 0.2
    avStroke.Thickness = 1
    avStroke.Parent = THUD.Avatar
    ...
    THUD.NameLbl.Size = UDim2.new(1, -62, 0, 14)
    ...
    THUD.BarBG.Position = UDim2.new(0, 54, 0, 40)
    THUD.BarBG.Size = UDim2.new(1, -62, 0, 2)
    ...
    THUD.HPText.Position = UDim2.new(1, -30, 0.5, -20)
    THUD.HPText.Size = UDim2.new(0, 30, 0, 14)
    ...
    local function IsVis(part, camP, targetChar)
        ...
        -- Offset slightly so we don't hit the back of the player's own hitbox
        rayDir = rayDir.Unit * (dist - 0.1)
        
        local visRP = RaycastParams.new()
        visRP.FilterType = Enum.RaycastFilterType.Exclude
        visRP.IgnoreWater = true
        ...
        visRP.FilterDescendantsInstances = filter
        
        -- Strict wallcheck: If it hits literally ANYTHING else, they are behind a wall.
        local result = workspace:Raycast(camP, rayDir, visRP)
                ...
                if cd.Hum.Health<=0 or not tar.Parent or not cd.Char or not cd.Char.Parent or not cd._wsValid then ClearTarget()
                    ...
                    if part then
                        ...
                        if hpFlr ~= _thudLastHP or dstFlr ~= _thudLastDist or tar.DisplayName ~= _thudLastName or wepName ~= _thudLastWep then
                            ...
                            THUD.Name.Text = tar.DisplayName
                            THUD.HP.Text = ("%d / %d HP"):format(hpFlr, mFloor(maxHP))
                            THUD.Dist.Text = ("[%dm]"):format(dstFlr)
                            THUD.Wep.Text = "Holding: " .. wepName
                        end
                        ...
                        if not onScreen then
                            ...
                            local cx = screenPos.X + 80 + animOffsetX
                            local cy = screenPos.Y - 24
                            ...
                            local w = mMax(180, nBounds + 80)
                            THUD.Container.Size = UDim2.new(0, w, 0, 52)
                            
                            THUD.DistLbl.Text = "WPN: " .. string.upper(wepName) .. " [" .. tostring(dstFlr) .. "M]"
                            
                            -- Tactical HP formatting
                            THUD.HPText.Text = tostring(hpFlr)
                            
                            -- Smooth bar fill using animation pct as well as actual health
                            local hpPct = mClamp(hpFlr / mMax(1, maxHP), 0, 1)
                            THUD.BarFill.Size = UDim2.new(hpPct * _thudAnimPct, 0, 1, 0)
                            
                            -- Tactical colors (white/grey normally, red when low)
                            if hpPct <= 0.3 then
                                THUD.BarFill.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
    ...
    VisSettings:AddLabel("Text Color"):AddColorPicker("G_TxtCol",{Default=C3N(1,1,1),
    ...
    Conns[#Conns+1] = RS.Heartbeat:Connect(function()
        ...
        if now - lastUIRefresh >= 0.5 then
            ...
            KaimWatermark:SetText("KAIM v12 | FPS: " .. tostring(fps) .. " | Ping: " .. mFloor(ping) .. "ms")
            lastUIRefresh = now
        end
    end)
        ...
        if not checkcaller() and S.Silent.On and S.Silent.Target then
            if method == "Raycast" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist" or method == "FindPartOnRay" then
                ...
                if tc and tc.Head then
                    ...
                    if S.Silent.HeadshotChance < 100 and mRand(1, 100) > S.Silent.HeadshotChance then
                        hitPart = tc.HRP or tc.Head
                    end
                    
                    if mRand(1, 100) <= S.Silent.HitChance then
